...
>(({ className, children, inputSize = "default", error, icon, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      "flex w-full items-center justify-between gap-2 rounded-lg bg-white border transition-all font-medium outline-none",
    ...
    {...props}
  >
    <span className="flex items-center gap-2 truncate [&_svg]:size-4 font-medium">
      {icon}
      {children}
    </span>
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="size-4 shrink-0 text-stone-600 transition-transform duration-200 data-[state=open]:rotate-180 font-medium" />
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      sideOffset={4}
      className={cn(
        "z-50 w-[var(--radix-select-trigger-width)] bg-white rounded-lg border border-stone-500 py-2",
...
>(({ className, children, icon, ...props }, ref) => (
  <div className="px-1.5 py-0.5">
    <SelectPrimitive.Item
      ...
      className={cn(
        "flex items-center justify-between px-4 py-2 rounded-md text-sm font-semibold cursor-pointer outline-none transition-colors",
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Separator
    ...
    className={cn("my-1 h-px bg-stone-400", className)}
    {...props}
  />
))
...
function SelectField({
  ...
  return (
    <div data-slot="select-field" className={cn("flex flex-col gap-1", className)}>
      {label && (
        <Label
          htmlFor={generatedId}
          required={required}
      ...
      {errorText ? (
        <p className={cn("mt-1 font-medium text-error-500", isLarge ? "text-sm font-medium" : "text-xs font-medium")}>
          {errorText}
        </p>
      ) : hintText ? (
        <p className={cn("mt-1 font-medium text-stone-700", isLarge ? "text-sm font-medium" : "text-xs font-medium")}>
          {hintText}
        </p>
      ) : null}
    </div>
